...
const GRAPHIC_NATIVE_TYPE_NAMES = {
  bar: 'bar',
  column: 'column',
  pie: 'pie',
  donut: 'donut',
...
module.exports = class ChartOptionsFactory {
  /**
   * @param {object} xAxisCategories
   * @param {object} dataColumn
   * @param {string} graphicTypeName
  ...
  constructor(
    ...
    let graphicOptionDto = new CustomBaseChart(
      graphicTypeName,
      titleText,
      descriptionChart,
      subtitleText,
    ...
    switch (graphicTypeName) {
      case GRAPHIC_NATIVE_TYPE_NAMES.bar:
        this.baseChart = new CustomBarChart(graphicOptionDto);
        break;
      case GRAPHIC_NATIVE_TYPE_NAMES.column:
        this.baseChart = new CustomBarChart(graphicOptionDto);
        break;
      case GRAPHIC_NATIVE_TYPE_NAMES.pie:
        this.baseChart = new CustomPieChart(graphicOptionDto);
        break;
      case GRAPHIC_NATIVE_TYPE_NAMES.donut:
        this.baseChart = new CustomDonutChart(graphicOptionDto);
        break;
      case GRAPHIC_NATIVE_TYPE_NAMES.spline:
        this.baseChart = new CustomBarChart(graphicOptionDto);
        break;
      case GRAPHIC_NATIVE_TYPE_NAMES.stacked:
        this.baseChart = new CustomStackedBarChart(graphicOptionDto);
        break;
      case GRAPHIC_NATIVE_TYPE_NAMES.stackedColumn:
...
class CustomBaseChart {
  ...
  /**
   * un set Highcontrast color settings */
  unSetHighConstrast() {
  ...
  async setBaseOptions() {
    this.graphicOptions = {
      ...
      tooltip: {
        ...
        pointFormat: `<tr><td style="color:{white};padding:0"><b>{series.name}:</b> </td><td style="padding:0">{point.y}</td></tr>}`,
        footerFormat: '</table>',
        useHTML: true,
      ...
      plotOptions: {
        series: {
          dataLabels: {
            ...
            style: {
              font: '14px',
              textOutline: !this.includeHighConstrast ? 0 : '1px white',
            },
          },
        },
        bar: {
...
class CustomBarChart extends CustomBaseChart {
  ...
  setBarGraphicOptions() {
    let barGraphicHeight = 800;
    this.graphicOptions['chart'].height = barGraphicHeight;
  }
}

class CustomPieChart extends CustomBaseChart {
  /**
  ...
  setPieGraphicOptions() {
    this.graphicOptions[
      'tooltip'
    ].pointFormat = `<tr><td style="color:{white};padding:0"><b>{series.name}:</b> </td><td style="padding:0">{point.y}</td></tr> <tr><td style="color:{white};padding:0"><b>Porcentaje:</b> {point.percentage:.1f} </td></tr>`;
  }
}

class CustomDonutChart extends CustomPieChart {
  /**
...
class CustomStackedBarChart extends CustomBarChart {
  /**
...
class CustomStackedColumnChart extends CustomBarChart {
  /**
   * Class to change some options to use stacked representation
   * for bars and columns.
   * @param { json } baseGraphicOptionsDto
...
// Have the chart options that correspond to highContrast
const CHART_OPTIONS = {
  highContrastOptions: {
    backgroundStyle: {
